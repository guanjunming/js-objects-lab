...
*/
game.party.push(pokemon[8]); // Blastoise
game.party.push(pokemon[34]); // Clefairy
game.party.push(pokemon[148]); // Dragonite
...
*/
game.completeGymByDifficulty = function (difficulty) {
  game.gyms.forEach((gym) => {
...
game.completeGymByDifficulty(6);
...
game.gymStatus = function () {
  ...
  game.gyms.forEach((gym) => {
    const key = gym.completed ? "completed" : "incomplete";
    gymTally[key] += 1;
  });
...
*/
game.completeGymByDifficulty(8);
...
pokemon.forEach((p) => {
  if (pokemonByTypes[p.type]) {
    pokemonByTypes[p.type].push(p);
  } else {
    pokemonByTypes[p.type] = [p];
  }
});
